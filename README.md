...
Required env vars (see `.env`): `VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`.

## Deploy (Vercel)

Static output: build command `npm run build`, output directory `dist`.
`vercel.json` rewrites every path to `index.html` so deep links such as `/app`
